...
from utils.torchplus.nn import GroupNorm
from utils.torchplus.nn import Empty
from utils.torchplus.nn import Sequential
from utils.torchplus.tools import change_default_args
# type hints
from model.type import RPNOutType
    ...
    def _make_layer(self, inplanes, planes, num_blocks, stride=1):
        if self._use_norm:
            if self._use_groupnorm:
                BatchNorm2d = change_default_args(num_groups=self._num_groups, eps=1e-3)(GroupNorm)
            else:
                BatchNorm2d = change_default_args(eps=1e-3, momentum=0.01)(nn.BatchNorm2d)
            Conv2d = change_default_args(bias=False)(nn.Conv2d)
            ConvTranspose2d = change_default_args(bias=False)(nn.ConvTranspose2d)
