...
from model.loss import prepare_loss_weights
from model.loss import LossNormType
from model.loss.loss_functions import WeightedSmoothL1LocalizationLoss
from model.loss.loss_functions import WeightedSoftmaxClassificationLoss
from utils.torchplus import metrics
from utils.torchplus.nn import one_hot
from utils import box_ops_torch

from typing import Union, List
from model.type import TrainSampleType, RPNOutType, SampleType, VoxelNetOutType


class VoxelNet(nn.Module):
    def __init__(self,
                 num_class,  # default=2
                 num_input_features,  # default=4
                 voxel_generator,
                 target_assigner,
                 vfe_block,
                 middle_block,
                 ...
                 use_rotate_nms: bool,
                 multiclass_nms: bool,
                 nms_pre_max_sizes: List[int],
                 nms_post_max_sizes: List[int],
        ...
        self.voxel_feature_extractor = vfe_block
        self.middle_feature_extractor = middle_block
        self.rpn = rpn_block
        ...
        self.rpn_metrics = metrics.PrecisionRecall(dim=-1,
    ...
    def loss(self, example: TrainSampleType, preds_dict: RPNOutType):
        ...
        loc_loss, cls_loss = create_loss(self._loc_loss_ftor,
                                         self._cls_loss_ftor,
                                         box_preds=box_preds,
                                         cls_preds=cls_preds,
    ...
    def network_forward(self,
                        ...
                        num_points: Union[torch.Tensor, np.ndarray],
                        coors: Union[torch.Tensor, np.ndarray],
                        batch_size: int) -> RPNOutType:
        """this function is used for subclass.
        you can add custom network architecture by subclass VoxelNet class
        and override this function.

        @param: voxels:  e.g. shape=[n_voxels=28919, pts_per_voxel=5, dim_xyzr=4]
        @param: num_points: e.g. shape=[n_voxels=28919]
        @param: coors:  e.g. [n_voxels=28919, 4]  4 -> [placeholder=0, x, y, z]
        @param: batch_size: e.g. 1
        ...
        # voxels: e.g. shape=[n_voxels=28919, pts_per_voxel=5, dim_xyzr=4]
        # num_points: e.g. shape=[n_voxels=28919]
        # coors: e.g. [n_voxels=28919, 4]  4 -> [placeholder=0, x, y, z]
        voxel_features = self.voxel_feature_extractor(voxels, num_points, coors)
        # voxel_features: e.g. shape=[28919, 3]
        ...
        # spatial_features: e.g. shape=[n_batch=1,bev_h=128,bev_w=160,bev_c=132]
    ...
    def forward(self, sample: Union[SampleType, TrainSampleType]):
        """module's forward should always accept dict and return loss.
        """
        voxels = sample["voxels"]  # e.g. (57123, 1, 4)  57123 is n of non-empyt voxel, 1 is max points per voxel, 4 is x,y,z,r
        num_points = sample["num_points"]  # (57123,)
        coors = sample["coordinates"]  # (57123, 3)
        if len(num_points.shape) == 2:  # multi-gpu
            num_voxel_per_batch = sample["num_voxels"].cpu().numpy().reshape(-1)
            voxel_list = []
            num_points_list = []
            coors_list = []
        ...
        if self.training:
            return self.loss(sample, preds_dict)
        else:
            self.start_timer("predict")
            with torch.no_grad():
                res = self.predict(sample, preds_dict)
            self.end_timer("predict")
            return res

    def predict(self, example: SampleType, preds_dict: RPNOutType) -> List[VoxelNetOutType]:
        ...
        for box_preds, cls_preds, dir_preds, a_mask, meta in zip(batch_box_preds, batch_cls_preds, batch_dir_preds, batch_anchors_mask, meta_list):
            ...
            if self._use_rotate_nms:
                nms_func = box_ops_torch.rotate_nms
            else:
                nms_func = box_ops_torch.nms
            ...
            if self._multiclass_nms:
                ...
                if not self._use_rotate_nms:
                    box_preds_corners = box_ops_torch.center_to_corner_box2d(
                        boxes_for_nms[:, :2],
                        boxes_for_nms[:, 2:4],
                        boxes_for_nms[:, 4])
                    boxes_for_nms = box_ops_torch.corner_to_standup_nd(box_preds_corners)
                ...
                for class_idx, score_thresh, pre_ms, post_ms, iou_th in zip(
                    ...
                    if selected is not None:
                        ...
                        selected_labels.append(torch.full([class_boxes[selected].shape[0]],
                                                          class_idx,
                                                          dtype=torch.int64,
                ...
                if top_scores.shape[0] != 0:
                    ...
                    boxes_for_nms = box_preds[:, [0, 1, 3, 4, 6]]
                    if not self._use_rotate_nms:
                        box_preds_corners = box_ops_torch.center_to_corner_box2d(boxes_for_nms[:, :2],
                                                                                 boxes_for_nms[:, 2:4],
                                                                                 boxes_for_nms[:, 4])
                        boxes_for_nms = box_ops_torch.corner_to_standup_nd(box_preds_corners)
            ...
            if selected_boxes.shape[0] != 0:
                ...
                if self._use_direction_classifier:
                    ...
                    dir_rot = box_ops_torch.limit_period(box_preds[..., 6] - self._dir_offset,
...
def get_direction_target(anchors,
    ...
    offset_rot = box_ops_torch.limit_period(rot_gt - dir_offset, 0, 2 * np.pi)
