...
def points_in_convex_polygon_jit(points, polygon, clockwise=True):
    ...
    polygon_shifted_index = [num_points_of_polygon - 1] + list(range(num_points_of_polygon - 1))
    polygon_shifted = np.zeros_like(polygon)
    for i, j in enumerate(polygon_shifted_index):
        polygon_shifted[:, i, :] = polygon[:, j, :]
    if clockwise:
        # numba.jit do no support such reorder with list
        # vec1 = polygon - polygon[:, [num_points_of_polygon - 1] + list(range(num_points_of_polygon - 1)), :]
        vec1 = polygon - polygon_shifted
    else:
        # vec1 = polygon[:, [num_points_of_polygon - 1] + list(range(num_points_of_polygon - 1)), :] - polygon
        vec1 = polygon_shifted - polygon
