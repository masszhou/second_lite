# import numba
import numpy as np
from spconv_lite.utils import non_max_suppression_cpu
from spconv_lite.utils import rotate_non_max_suppression_cpu
...
def rotate_nms_cc(dets, thresh):
    scores = dets[:, 5]
    order = scores.argsort()[::-1].astype(np.int32)  # highest->lowest
    dets_corners = box_ops_np.center_to_corner_box2d(dets[:, :2], dets[:, 2:4], dets[:, 4])
    dets_standup = box_ops_np.corner_to_standup_nd(dets_corners)
    standup_iou = box_ops_np.iou_jit(dets_standup, dets_standup, eps=0.0)
