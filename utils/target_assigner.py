...
from utils.target_ops import create_target_np

from typing import List, Optional
from model.type import BaseBoxCoder, BaseAnchorGenerator, BaseRegionSimilarityCalculator


class TargetAssigner:
    def __init__(self,
                 box_coder: BaseBoxCoder,
                 anchor_generators: List[BaseAnchorGenerator],
                 classes: List[str],
                 feature_map_sizes: List,
                 region_similarity_calculators: List[BaseRegionSimilarityCalculator],
                 positive_fraction: Optional[int] = None,
                 sample_size: int = 512,
    ...
    def assign(self,
        ...
        if self._assign_per_class:
            return self.assign_per_class(anchors_dict, gt_boxes, anchors_mask, gt_classes, gt_names,
                                         importance=importance)
        else:
            return self.assign_all(anchors, gt_boxes, anchors_mask, gt_classes, matched_thresholds,
                                   unmatched_thresholds, importance=importance)
    ...
    def assign_per_class(self,
        ...
        for class_name, anchor_dict in anchors_dict.items():
            ...
            def similarity_fn(anchors, gt_boxes):
                anchors_rbv = anchors[:, [0, 1, 3, 4, 6]]
                gt_boxes_rbv = gt_boxes[:, [0, 1, 3, 4, 6]]
                return self._sim_calcs[anchor_gene_idx].compare(anchors_rbv, gt_boxes_rbv)
        ...
        targets_dict["bbox_targets"] = np.concatenate(
            [v.reshape(-1, self.box_coder.code_size) for v in targets_dict["bbox_targets"]],
            axis=0)
    ...
    def generate_anchors(self, feature_map_size: List[int]):
        anchors_list = []
        ...
        if not self._feature_map_sizes:  # if list is empty
