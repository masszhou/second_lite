...
def draw_box_in_bev(img,
    ...
    if boxes.shape[1] == 9:
        ...
        for box in boxes:
            ...
            if np.isnan(velo_unified[0]):
                continue
            velo_unified = velo_unified * np.array(img.shape[:2])[::-1] / (coors_range[3:5] - coors_range[:2])
            center = box[:2] - coors_range[:2]
            center = center * np.array(img.shape[:2])[::-1] / (coors_range[3:5] - coors_range[:2])
    ...
    if labels is not None:
        if label_color is None:
            ...
            label_color = np.tile(np.array(label_color).reshape(1, 3), [bev_lines.shape[0], 1])
